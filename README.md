...
Некоторые отличительные особенности данной версии шаблона:
* В диссертациях, представляемых к защите в новых советах, нет автореферата. Вместо него пишется реферат на русском и английском языках. Реферат похож по структуре на "ваковский" автореферат, однако несколько превосходит его по объёму и является частью диссертации. Таким образом, с помощью LaTeXa следует собирать единственный файл - `dissertation.tex`.
* Список литературы является общим для реферата и основной части диссертации. Поэтому номера ссылок на статьи автора из реферата относятся к "большому" списку литературы в конце диссертации, а не к списку опубликованных статей автора.
* Для сборки рекомендуется использовать XeLaTeX и BibLaTeX (с бэкендом Biber).
...
[**Примеры компиляции оригинального шаблона**](https://github.com/AndreyAkinshin/Russian-Phd-LaTeX-Dissertation-Template/releases/latest).
